...
# -----------------------------------------------------------------------------------------
def parse_fddb_gt(dets_file_name, isEllipse=False):
    # -----------------------------------------------------------------------------------------
    """
    ...
    for line in fid:
        ...
        if start_det_count:
            ...
            major_axis_radius = float(detection[0])
            minor_axis_radius = float(detection[1])
            angle = float(detection[2])
            center_x = float(detection[3])
            center_y = float(detection[4])
            calc_x = math.sqrt(
                major_axis_radius ** 2 * math.cos(angle) ** 2
                + minor_axis_radius ** 2 * math.sin(angle) ** 2
            )
            calc_y = math.sqrt(
                major_axis_radius ** 2 * math.sin(angle) ** 2
...
def convert_wider_annots(dataset_name, annotation_file, img_dir, json_file):
    ...
    wider_annot_dict = parse_fddb_gt(ann_file)  # [im-file] = [[x,y,w,h], ...]
